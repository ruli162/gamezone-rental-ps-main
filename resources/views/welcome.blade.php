    ...
    <main class="flex-grow pt-32 pb-20 w-full max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                ...
                <div data-aos="fade-up" data-aos-duration="1000" class="glass-card glass-card-hover rounded-3xl p-2 group relative overflow-hidden animate-glow-border">
                    ...
                    <div class="bg-ps-surface/90 rounded-[1.5rem] p-6 h-full flex flex-col relative z-10 border border-white/5">
                        ...
                        <div class="flex justify-between items-start mb-6 gap-2">
                            ...
                            @if(isset($stock['ps5']) && $stock['ps5'] > 0)
                                <span class="px-3 py-1 bg-green-500/20 text-green-400 text-xs font-bold rounded-lg border border-green-500/30 whitespace-nowrap">{{ $stock['ps5'] }} Unit Tersedia</span>
                            @else
                                @if(isset($nextAvailable['ps5']) && $nextAvailable['ps5'])
                                    <span class="px-3 py-1 bg-yellow-500/20 text-yellow-400 text-xs font-bold rounded-lg border border-yellow-500/30 whitespace-nowrap"
                                          x-data="{ 
                                              end: {{ \Carbon\Carbon::parse($nextAvailable['ps5'])->timestamp * 1000 }},
                        ...
                        <div class="pt-4 border-t border-white/10 flex justify-between items-end">
                            <div>
                                ...
                                <span class="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-cyan-300 font-display">{{ isset($categories['ps5']) ? ($categories['ps5']->price_per_hour / 1000) . 'K' : '10K' }}</span>
                            </div>
                ...
                <div data-aos="fade-up" data-aos-duration="1000" data-aos-delay="100" class="glass-card glass-card-hover rounded-3xl p-2 group relative overflow-hidden">
                    ...
                    <div class="bg-ps-surface/90 rounded-[1.5rem] p-6 h-full flex flex-col relative z-10 border border-white/5">
                        ...
                        <div class="flex justify-between items-start mb-6 gap-2">
                            ...
                            @if(isset($stock['ps4']) && $stock['ps4'] > 0)
                                <span class="px-3 py-1 bg-green-500/20 text-green-400 text-xs font-bold rounded-lg border border-green-500/30 whitespace-nowrap">{{ $stock['ps4'] }} Unit Tersedia</span>
                            @else
                                @if(isset($nextAvailable['ps4']) && $nextAvailable['ps4'])
                                    <span class="px-3 py-1 bg-yellow-500/20 text-yellow-400 text-xs font-bold rounded-lg border border-yellow-500/30 whitespace-nowrap"
                                          x-data="{ 
                                              end: {{ \Carbon\Carbon::parse($nextAvailable['ps4'])->timestamp * 1000 }},
                                              now: new Date().getTime(),
                                              distance: 0,
                                              text: 'Menghitung...',
                                              init() {
                        ...
                        <div class="pt-4 border-t border-white/10 flex justify-between items-end">
                            <div>
                                <span class="text-sm text-slate-300 font-bold tracking-widest font-display mb-1 block">TARIF/JAM</span>
                                <span class="text-4xl font-black text-white font-display">{{ isset($categories['ps4']) ? ($categories['ps4']->price_per_hour / 1000) . 'K' : '6K' }}</span>
                            </div>
                ...
                <div data-aos="fade-up" data-aos-duration="1000" data-aos-delay="200" class="glass-card glass-card-hover rounded-3xl p-2 group relative overflow-hidden">
                    ...
                    <div class="bg-ps-surface/90 rounded-[1.5rem] p-6 h-full flex flex-col relative z-10 border border-white/5">
                        ...
                        <div class="flex justify-between items-start mb-6 gap-2">
                            ...
                            @if(isset($stock['ps3']) && $stock['ps3'] > 0)
                                <span class="px-3 py-1 bg-green-500/20 text-green-400 text-xs font-bold rounded-lg border border-green-500/30 whitespace-nowrap">{{ $stock['ps3'] }} Unit Tersedia</span>
                            @else
                                @if(isset($nextAvailable['ps3']) && $nextAvailable['ps3'])
                                    <span class="px-3 py-1 bg-yellow-500/20 text-yellow-400 text-xs font-bold rounded-lg border border-yellow-500/30 whitespace-nowrap"
                                          x-data="{ 
                                              end: {{ \Carbon\Carbon::parse($nextAvailable['ps3'])->timestamp * 1000 }},
                                              now: new Date().getTime(),
                                              distance: 0,
                                              text: 'Menghitung...',
                                              init() {
                        ...
                        <div class="pt-4 border-t border-white/10 flex justify-between items-end">
                            <div>
                                <span class="text-sm text-slate-300 font-bold tracking-widest font-display mb-1 block">TARIF/JAM</span>
                                <span class="text-4xl font-black text-white font-display">{{ isset($categories['ps3']) ? ($categories['ps3']->price_per_hour / 1000) . 'K' : '4K' }}</span>
                            </div>
                ...
                <div data-aos="fade-up" data-aos-duration="1000" data-aos-delay="300" class="glass-card rounded-3xl p-2 text-center relative overflow-hidden group">
                    ...
                    <div class="h-full rounded-[1.5rem] p-6 flex flex-col justify-center items-center relative z-10 transition-transform group-hover:scale-105 duration-500">
                        ...
                        <span class="px-3 py-1 bg-black/30 backdrop-blur-md text-yellow-300 text-sm font-bold rounded-lg mb-4 border border-yellow-400/30 font-display">KODE: {{ $promo->code }}</span>
                        
                        <h3 class="text-2xl font-display font-black text-white mb-2 leading-tight uppercase drop-shadow-md">{{ $promo->description ?? 'PROMO SPESIAL' }}</h3>
                        ...
                        <div class="mb-6">
                            ...
                            <span class="text-3xl font-black text-yellow-300 font-display drop-shadow-[0_0_15px_rgba(253,224,71,0.6)]">{{ number_format($promo->discount_amount, 0, ',', '.') }} IDR</span>
                        </div>
                        
                        <a href="/booking?promo={{ $promo->code }}" class="w-full bg-white text-blue-900 hover:bg-yellow-300 py-3 font-black rounded-xl flex items-center justify-center gap-2 transition-all active:scale-95 font-display tracking-widest shadow-xl text-sm">
                            KLAIM SEKARANG
                        </a>
                    </div>
                </div>
        ...
        <footer class="mt-24 border-t border-white/10 bg-ps-surface/50 backdrop-blur-xl rounded-t-[3rem] px-8 py-12 relative overflow-hidden">
            ...
            <div class="text-center mt-12 pt-8 border-t border-white/10 text-slate-500 font-medium tracking-widest font-display">
                &copy; {{ date('Y') }} GAMEZONE INC. ALL RIGHTS RESERVED.
            </div>
        </footer>
