...
<head>
    ...
    <title>Lupa Kata Sandi - GAMEZONE</title>
...
<body class="bg-[#030712] text-slate-300 font-body antialiased min-h-screen flex items-center justify-center relative">
    ...
    <div class="w-full max-w-md p-8 relative z-10">
        ...
        <div class="bg-slate-900/40 backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 shadow-2xl relative overflow-hidden">
            <h2 class="text-2xl font-display font-bold text-white mb-2 text-center">Lupa Kata Sandi?</h2>
            <p class="text-slate-400 text-sm text-center mb-8">Masukkan email akun Anda. Kami akan mengirimkan tautan aman untuk membuat sandi baru.</p>
            ...
            <form method="POST" action="{{ route('password.email') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="contoh@email.com" required class="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3.5 text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all font-medium text-sm">
                </div>
            ...
            <div class="mt-8 text-center space-y-4">
                <a href="{{ route('login') }}" class="text-sm font-semibold text-slate-500 hover:text-white transition block">← Kembali ke Login</a>
            </div>
        </div>
    </div>
</body>
