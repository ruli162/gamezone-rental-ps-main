        ...
        <div class="overflow-x-auto text-left" x-data="{ showEditModal: false, editForm: { id: '', inventory_id: '', status: '' }, filter: 'all' }">
            ...
            <table class="w-full">
                ...
                <tbody>
                    ...
                    <tr x-show="filter === 'all' || (filter === 'lunas' && '{{ $booking->payment->payment_status ?? 'pending' }}' === 'success') || (filter === 'belum' && '{{ $booking->payment->payment_status ?? 'pending' }}' !== 'success')" class="border-b border-slate-800/50 hover:bg-slate-800/30 transition">
                        ...
                        <td class="py-4 px-4 text-slate-400">
                            ...
                            @if($booking->status == 'active')
                                <div class="mt-1" x-data="{
                                    end: new Date('{{ \Carbon\Carbon::parse($booking->end_time)->toISOString() }}').getTime(),
                        ...
                        <td class="py-4 px-4 text-right flex justify-end gap-2">
                            @if($booking->status == 'pending')
                                <form action="{{ route('admin.bookings.process', $booking->id) }}" method="POST" class="inline">
                                    @csrf
                            ...
                            @elseif($booking->status == 'active')
                                <form action="{{ route('admin.bookings.finish', $booking->id) }}" method="POST" class="inline">
                                    @csrf
                            ...
                            <button @click="showEditModal = true; editForm.id = '{{ $booking->id }}'; editForm.inventory_id = '{{ $booking->inventory_id }}'; editForm.status = '{{ $booking->status }}'" class="bg-slate-700/50 hover:bg-slate-700 border border-slate-600 transition-colors text-white px-3 py-1.5 rounded-lg text-sm font-bold">Edit</button>
                        ...
                        <div class="mb-6">
                            ...
                            <select name="inventory_id" x-model="editForm.inventory_id" class="w-full bg-slate-950 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                                ...
                                @foreach($inventories as $inv)
                                    <option value="{{ $inv->id }}">{{ $inv->name }} ({{ $inv->status }})</option>
                                @endforeach
                            </select>
                        </div>
