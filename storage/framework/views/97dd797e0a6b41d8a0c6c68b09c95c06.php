        ...
        <div class="w-full md:w-1/3 space-y-6">
            <div class="glass-panel p-8 rounded-3xl relative overflow-hidden text-center border-t border-white/10">
                ...
                <div class="w-24 h-24 mx-auto bg-gradient-to-br from-blue-600 to-indigo-800 rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(59,130,246,0.3)] mb-4 border-2 border-white/10">
                    <span class="text-3xl font-black text-white font-display"><?php echo e(substr($user->name, 0, 1)); ?></span>
                </div>
                
                <h2 class="text-xl font-display font-bold text-white"><?php echo e($user->name); ?></h2>
                <p class="text-slate-400 text-sm mb-6">
                    <?php if(str_ends_with($user->email, '@guest.gamezone.com')): ?>
                ...
                <div class="bg-slate-900/50 rounded-xl p-4 text-left border border-white/5 space-y-3">
                    <div>
                        ...
                        <p class="text-white font-medium text-sm"><?php echo e($user->phone_number ?? '-'); ?></p>
                    </div>
                    <div>
                        <span class="text-xs text-slate-500 font-bold uppercase tracking-wider">Bergabung Sejak</span>
                        <p class="text-white font-medium text-sm"><?php echo e($user->created_at->translatedFormat('d M Y')); ?></p>
                    </div>
                </div>
                ...
                <div class="mt-4 bg-gradient-to-r from-blue-600/20 to-cyan-600/20 border border-blue-500/30 rounded-xl p-4 text-center">
                    ...
                    <span class="text-white font-bold text-sm">Total Belanja: Rp <?php echo e(number_format($bookings->where('status', '!=', 'cancelled')->sum('total_price'), 0, ',', '.')); ?></span>
                </div>
                ...
                <div class="glass-panel p-6 rounded-2xl flex flex-col sm:flex-row justify-between sm:items-center gap-4 hover:border-blue-500/30 transition-colors">
                    <div>
                        <div class="flex items-center gap-3 mb-2">
                            <span class="px-2 py-1 bg-slate-800 text-slate-300 rounded text-xs font-bold border border-white/5 font-mono">
                                <?php echo e($booking->payment->transaction_reference ?? 'TRX-...'); ?>
                        ...
                        <h4 class="text-lg font-bold text-white font-display mb-1">
                            Sewa <?php echo e($booking->inventory->category->name ?? 'Konsol'); ?>

                        </h4>
                        <p class="text-sm text-slate-400 mb-2">
                            <?php echo e(\Carbon\Carbon::parse($booking->start_time)->translatedFormat('d M Y, H:i')); ?> - <?php echo e(\Carbon\Carbon::parse($booking->end_time)->translatedFormat('d M Y, H:i')); ?>
                    ...
                    <div class="sm:text-right flex flex-row sm:flex-col justify-between sm:justify-center items-center sm:items-end">
                        <div class="mb-2">
                            ...
                            <span class="text-xl font-black text-blue-300 font-display">Rp <?php echo e(number_format($booking->total_price, 0, ',', '.')); ?></span>
                        </div>
                        <a href="/booking/<?php echo e($booking->id); ?>/receipt" class="text-sm font-bold text-cyan-400 hover:text-cyan-300 underline underline-offset-4">Lihat Struk</a>
                    </div>
                </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\COMPUTER\Downloads\gamezone-rental-ps-main\gamezone-rental-ps-main\resources\views/frontend/dashboard.blade.php ENDPATH**/ ?>