...
<div class="glass-panel rounded-2xl border border-white/5 p-8 text-center text-slate-500">
    ...
    <?php else: ?>
        <form action="<?php echo e(route('admin.customers.bulkDestroy')); ?>" method="POST" id="bulk-delete-form">
            <?php echo csrf_field(); ?>
                ...
                <table class="w-full">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-slate-800/50 hover:bg-slate-800/80 transition group">
                            <td class="py-4 px-6">
                                <input type="checkbox" name="customer_ids[]" value="<?php echo e($c->id); ?>" class="customer-checkbox w-5 h-5 rounded border-slate-600 bg-slate-800 text-blue-500 focus:ring-blue-500 focus:ring-offset-slate-900 cursor-pointer transition">
                            </td>
                            <td class="py-4 px-6 text-white font-medium group-hover:text-blue-400 transition"><?php echo e($c->name); ?></td>
                            <td class="py-4 px-6">
                                <span class="block text-slate-300 font-medium"><?php echo e($c->phone_number); ?></span>
                                <span class="block text-slate-500 text-sm mt-0.5"><?php echo e($c->email); ?></span>
                            </td>
                            <td class="py-4 px-6 text-slate-400">
                                <span class="bg-slate-800 px-3 py-1 rounded-full text-xs border border-slate-700"><?php echo e($c->created_at->format('d M Y')); ?></span>
                            </td>
                            <td class="py-4 px-6 text-right">
                                <a href="javascript:void(0)" onclick="deleteSingle(<?php echo e($c->id); ?>)" class="text-slate-500 hover:text-red-400 transition-colors p-2 inline-block rounded-lg hover:bg-red-500/10">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\COMPUTER\Downloads\gamezone-rental-ps-main\gamezone-rental-ps-main\resources\views/backend/customers.blade.php ENDPATH**/ ?>