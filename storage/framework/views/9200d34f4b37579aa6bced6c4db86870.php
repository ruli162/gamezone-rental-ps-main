...
<div class="max-w-4xl mx-auto mt-8">
    ...
    <div class="glass-panel p-8 md:p-10 rounded-[2rem] relative overflow-hidden border border-white/10">
        ...
        <form action="/booking" method="POST" class="relative z-10" x-data="{ 
            unit: '<?php echo e(request('category') ?: (($stock['ps5'] > 0) ? 'ps5' : (($stock['ps4'] > 0) ? 'ps4' : (($stock['ps3'] > 0) ? 'ps3' : 'ps5')))); ?>', 
            rental_type: 'main_di_tempat',
            duration: 1, 
            payment_method: 'otomatis',
            promo_code: '<?php echo e(request('promo')); ?>',
            promo_error: '',
            promos: <?php echo \Illuminate\Support\Js::from($promos ?? [])->toHtml() ?>,
            ...
            <div id="alert-box" class="mb-6 bg-red-500/20 border border-red-500/50 text-red-100 p-4 rounded-xl flex gap-3 items-start">
                ...
                <div>
                    ...
                    <p class="text-sm mt-1 opacity-90"><?php echo e(session('error')); ?></p>
                </div>
            </div>
            <?php endif; ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="<?php echo e(Auth::check() ? Auth::user()->name : ''); ?>" required class="w-full bg-slate-900/60 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all font-medium" placeholder="Masukkan nama Anda">
                    </div>
                    
                    <div>
                        <label class="block text-sm font-bold tracking-wide text-slate-400 mb-2">NOMOR WHATSAPP</label>
                        <input type="text" name="phone" value="<?php echo e(Auth::check() ? Auth::user()->phone_number : ''); ?>" required class="w-full bg-slate-900/60 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all font-medium" placeholder="08...">
                    </div>
                </div>
                        ...
                        <div x-data="{ openDropdown: false }" class="relative">
                            <div @click="openDropdown = !openDropdown" @click.away="openDropdown = false" class="w-full bg-slate-900/80 border border-slate-700 hover:border-blue-500 rounded-xl px-4 py-3 text-white cursor-pointer flex justify-between items-center transition-all shadow-lg focus:outline-none focus:ring-2 focus:ring-blue-500" tabindex="0">
                                <div class="flex items-center gap-3 font-bold">
                                    ...
                                    <span x-text="unit === 'ps5' ? 'PlayStation 5 <?php echo e($stock['ps5'] > 0 ? '('.$stock['ps5'].' Tersedia)' : '(Penuh/Habis)'); ?>' : (unit === 'ps4' ? 'PlayStation 4 Pro <?php echo e($stock['ps4'] > 0 ? '('.$stock['ps4'].' Tersedia)' : '(Penuh/Habis)'); ?>' : 'PlayStation 3 Retro <?php echo e($stock['ps3'] > 0 ? '('.$stock['ps3'].' Tersedia)' : '(Penuh/Habis)'); ?>')"></span>
                                </div>
                            ...
                            <div x-show="openDropdown" x-transition.opacity.duration.200ms class="absolute z-50 w-full mt-2 bg-slate-800 border border-slate-600 rounded-xl shadow-2xl overflow-hidden" style="display: none;">
                                <div @click="unit = 'ps5'; openDropdown = false" class="px-4 py-3 hover:bg-blue-600 cursor-pointer transition-colors flex items-center gap-3 border-b border-slate-700/50" :class="unit === 'ps5' ? 'bg-blue-600/50 text-white' : 'text-slate-300'">
                                    ...
                                    <span class="text-xs px-2 py-1 rounded-full ml-auto <?php echo e($stock['ps5'] > 0 ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'); ?>"><?php echo e($stock['ps5'] > 0 ? $stock['ps5'].' Tersedia' : 'Penuh/Habis'); ?></span>
                                </div>
                                <div @click="unit = 'ps4'; openDropdown = false" class="px-4 py-3 hover:bg-blue-600 cursor-pointer transition-colors flex items-center gap-3 border-b border-slate-700/50" :class="unit === 'ps4' ? 'bg-blue-600/50 text-white' : 'text-slate-300'">
                                    ...
                                    <span class="text-xs px-2 py-1 rounded-full ml-auto <?php echo e($stock['ps4'] > 0 ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'); ?>"><?php echo e($stock['ps4'] > 0 ? $stock['ps4'].' Tersedia' : 'Penuh/Habis'); ?></span>
                                </div>
                                <div @click="unit = 'ps3'; openDropdown = false" class="px-4 py-3 hover:bg-blue-600 cursor-pointer transition-colors flex items-center gap-3" :class="unit === 'ps3' ? 'bg-blue-600/50 text-white' : 'text-slate-300'">
                                    ...
                                    <span class="text-xs px-2 py-1 rounded-full ml-auto <?php echo e($stock['ps3'] > 0 ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'); ?>"><?php echo e($stock['ps3'] > 0 ? $stock['ps3'].' Tersedia' : 'Penuh/Habis'); ?></span>
                                </div>
                            </div>
            ...
            <div class="space-y-4 mb-8">
                ...
                <div class="mb-4 bg-slate-900/80 border border-yellow-500/30 rounded-xl p-4 shadow-inner">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <button type="button" @click="promo_code = '<?php echo e($promo->code); ?>'" class="text-xs bg-yellow-500/10 hover:bg-yellow-500/30 text-yellow-300 border border-yellow-500/40 px-3 py-2 rounded-lg font-bold transition-colors cursor-pointer text-left">
                            <span class="block text-[10px] text-yellow-400/70 mb-0.5"><?php echo e($promo->description ?? 'Diskon Spesial'); ?></span>
                            <?php echo e($promo->code); ?> <span class="text-white">(-Rp<?php echo e(number_format($promo->discount_amount, 0, ',', '.')); ?>)</span>
                        </button>
                        <?php endif; ?>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\COMPUTER\Downloads\gamezone-rental-ps-main\gamezone-rental-ps-main\resources\views/frontend/booking.blade.php ENDPATH**/ ?>